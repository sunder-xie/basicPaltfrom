...
$(document).ready(function() {
	...
	$('#hideFrameBack').bind('load', promgtMsgBack);
});
...
function submit(){
	$.ajax({
		...
		success:function(data) {
			   if(isNaN(data)){
				   $.messager.alert('Tips',"登陆超时,请重新登录后再处理！",'error');
			   }else if (data==0) {
					$.ajax({
	...
	$.messager.confirm('Confirm', '确认回退事务?', function(r) {
		if (r) {
			...
			if('fix'==str){
				$.messager.progress();
				var form = window.document.forms[0];
				form.action = appUrl+ "/wfe/eventAction!backEvent.jspa?curStaIdBack=" + staId;
				form.target = "hideFrameBack";
				form.submit();
			}
		}
	});
...
//经销商开户-回退处理调用
function promgtMsgBack() {
	//$.messager.progress('close');
	var hideFrameWfe = document.getElementById("hideFrameBack");
	var failResult = hideFrameWfe.contentWindow.failResult;
	var successResult = hideFrameWfe.contentWindow.successResult;
	if (failResult) {
		$.messager.alert('Tips', failResult, 'error');
		...
		$.ajax({
			...
			async : false,
			url : appUrl1
					+ "/crmPlatform/kunnrAction!modifyProcessVariable.jspa?time="
					+ new Date(),
			data : {
				eventId : $("#eventId").val()
			},
			...
			error : function() {
				$.messager.progress('close');
				$.messager.alert('Tips',"修改流程参数错误!请联系系统管理员", 'error');
				return;
			}
		});
